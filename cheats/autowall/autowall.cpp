...
bool autowall::is_breakable_entity(IClientEntity* e)
{
	...
	if ((client_class->m_pNetworkName[1] == 'B' && client_class->m_pNetworkName[9] == 'e' && client_class->m_pNetworkName[10] == 'S' && client_class->m_pNetworkName[16] == 'e')
		|| (client_class->m_pNetworkName[1] != 'B' || client_class->m_pNetworkName[5] != 'D'))
...
float autowall::GetDamag(const Vector& eye_pos, player_t* pLocal, const Vector& vecPoint, FireBulletData_t* pDataOut)
{
...
void autowall::ClipTraceToPlayers(const Vector& vecAbsStart, const Vector& vecAbsEnd, const unsigned int fMask, ITraceFilter* pFilter, trace_t* pTrace, const float flMinRange)
{
...
bool autowall::TraceToExit(trace_t& enterTrace, trace_t& exitTrace, const Vector& vecPosition, const Vector& vecDirection, player_t* pClipPlayer)
{
...
bool autowall::HandleBulletPenetration(player_t* pLocal, weapon_info_t* pWeaponData, const surfacedata_t* pEnterSurfaceData, FireBulletData_t& data)
{
