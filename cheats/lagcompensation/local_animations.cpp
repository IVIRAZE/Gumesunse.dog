...
void local_animations::run(ClientFrameStage_t stage)
{
	...
	if (!fakelag::get().condition && c_config::get()->auto_check(c_config::get()->i["rage_fd_enabled"], c_config::get()->i["rage_fd_enabled_style"]))
	{
		if (stage == FRAME_NET_UPDATE_END)
		{
			...
			g_ctx.local()->get_animlayers()[12].m_flWeight = 1.0f;
	...
	else if (stage == FRAME_RENDER_START)
	{
		...
		if (g_cfg.misc.slidewalk == 2) {
			...
			if (g_ctx.globals.loaded_script)
				for (auto& current : c_lua::get().hooks.getHooks("pre_render"))
					current.func();
		} 
		update_fake_animations();
		update_local_animations(animstate);
		...
		if (c_config::get()->b["moonwalk"]) {
			g_ctx.local()->m_flPoseParameter()[7] = 0;
		}
		if (c_config::get()->b["backwardleg"]) {
			g_ctx.local()->m_flPoseParameter()[0] = 0;
		}
	}
}
...
void local_animations::update_prediction_animations()
{
	...
	if (!alloc && !change && !reset)
	{
		...
		g_ctx.local()->setup_bones_fixed(g_ctx.globals.prediction_matrix, BONE_USED_BY_ANYTHING);
