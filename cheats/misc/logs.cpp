...
void eventlogs::paint_traverse()
{
	...
	for (size_t i = 0; i < logs.size(); i++)
	{
		...
		//render::get().text(fonts[NAME], log.x, last_y + log.y - logs_size_inverted * 14, log.color, HFONT_CENTERED_NONE, log.message.c_str());
...
void eventlogs::events(IGameEvent* event)
{
	...
	if (c_config::get()->b["misc_log_dam"]  && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		{
			ss << crypt_str("Hit ") << userid_info.szName << crypt_str("'s") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup"))) << crypt_str(" for ") << event->GetInt(crypt_str("dmg_health ")) << crypt_str(" (") << event->GetInt(crypt_str("health")) << crypt_str(" remaining)");
	...
	if (c_config::get()->b["misc_log_wep"] && !strcmp(event->GetName(), crypt_str("item_purchase")))
	{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_BOMB] && !strcmp(event->GetName(), crypt_str("bomb_beginplant")))
	{
		...
		std::stringstream ss;
		ss << userid_info.szName << crypt_str("'s has began planting the bomb");
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_BOMB] && !strcmp(event->GetName(), crypt_str("bomb_begindefuse")))
	{
		...
		std::stringstream ss;
		ss << userid_info.szName << crypt_str("'s has began defusing the bomb ") << (event->GetBool(crypt_str("haskit")) ? crypt_str("with defuse kit") : crypt_str("without defuse kit"));
...
void eventlogs::add(std::string text, bool full_display)
{
	...
	m_cvar()->ConsoleColorPrintf(Color(D3DCOLOR_RGBA(153, 225, 1, 255)), crypt_str("[gamesense] ")); //-V807
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		auto log = crypt_str("[gamesense] ") + text;
		chat->chat_print(log.c_str());
	}
}

void eventlogs::luaerror(std::string text, bool full_display)
{
	logs.emplace_front(loginfo_t(util::epoch_time(), text, g_cfg.misc.log_color));
	...
	m_cvar()->ConsoleColorPrintf(Color::Red, text.c_str());
	m_cvar()->ConsolePrintf(crypt_str("\n"));
}