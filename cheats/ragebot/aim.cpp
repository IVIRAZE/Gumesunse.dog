...
int aim::get_minimum_damage(bool visible, int health)
{
	...
	minimum_damage = math::clamp(c_config::get()->i["Ragebot.minimum_damage"], 1, health);
...
void aim::PredictiveQuickStop(CUserCmd* cmd, int idx) {
	auto i = 0; i < 8; i++;
	if (!should_stop && g_cfg.ragebot.weapon[i].autostop_modifiers[AUTOSTOP_PREDICTIVE]) {
		auto max_speed = 260.0f;
		auto weapon_info = g_ctx.globals.weapon->get_csweapon_info();
		...
		for (auto& target : targets)
		{
			...
			should_stop = GetTicksToShoot() <= GetTicksToStop() || g_cfg.ragebot.weapon[i].autostop_modifiers[1] && !g_ctx.globals.weapon->can_fire(true);
			break;
		}
	}
}
...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	if (!should_stop)
		return true;
	auto i = 0; i < 8; i++;
	if (!g_cfg.ragebot.weapon[i].autostop)
		return true;
	...
	if (!g_cfg.ragebot.weapon[i].autostop_modifiers[AUTOSTOP_BETWEEN_SHOTS] && !g_ctx.globals.weapon->can_fire(false))
		return true;
...
float aim::GetBodyScale(player_t* player)
{
	...
	if (g_cfg.ragebot.weapon[i].static_point_scale)
		return std::clamp(g_cfg.ragebot.weapon[i].body_scale, 0.f, 0.75f);
...
float aim::GetHeadScale(player_t* player)
{
	...
	if (g_cfg.ragebot.weapon[i].static_point_scale)
		return std::clamp(g_cfg.ragebot.weapon[i].head_scale, 0.f, 0.80f);
...
int aim::GetDamage(int health) {
	...
	auto i = 0; i < 8; i++;
	auto minimum_damage = key_binds::get().get_key_bind_state(4 + i) ? g_cfg.ragebot.weapon[i].minimum_override_damage : g_cfg.ragebot.weapon[i].minimum_damage;
...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
	...
	for (auto& point : points)
	{
		...
		if (body_hitboxes && (point.hitbox < HITBOX_PELVIS || point.hitbox > HITBOX_UPPER_CHEST))
		{
			...
			if (g_cfg.ragebot.weapon[i].prefer_body_aim && best_damage >= 1)
				break;
		}

		FireBulletData_t fire_data = { };

		fire_data.damage = autowall::GetDamag(shoot_position, g_ctx.local(), point.point, &fire_data);
		...
		if (fire_data.damage >= current_minimum_damage && fire_data.damage >= best_damage)
		{
			...
			if (fire_data.damage >= current_minimum_damage && fire_data.damage >= best_damage)
			{
				should_stop = GetTicksToShoot() <= GetTicksToStop() || g_cfg.ragebot.weapon[i].autostop_modifiers.at(1) && !g_ctx.globals.weapon->can_fire(true);


				if (((record->flags & FL_ONGROUND && g_cfg.ragebot.weapon[i].prefer_safe_points && point.safe) || (record->flags & FL_ONGROUND && point.hitbox >= HITBOX_PELVIS && point.hitbox <= HITBOX_UPPER_CHEST) || ((!(record->flags & FL_ONGROUND) || record->shot) && point.hitbox == HITBOX_HEAD)) && fire_data.damage >= record->player->m_iHealth())
				{
					best_damage = fire_data.damage;
			...
			//
			else if (g_cfg.ragebot.weapon[i].prefer_safe_points) {
				if (point.safe && point.hitbox >= HITBOX_PELVIS && point.hitbox <= HITBOX_UPPER_CHEST && g_cfg.ragebot.weapon[i].prefer_body_aim) {
			...
			else if (point.hitbox >= HITBOX_PELVIS && point.hitbox <= HITBOX_UPPER_CHEST && g_cfg.ragebot.weapon[i].prefer_body_aim) {
				if ((fire_data.damage > best_damage && body_hitboxes != point.hitbox) || fire_data.damage > best_damage + 20.f) {
					if (fire_data.damage >= minimum_damage)
					{
						best_damage = best_damage = fire_data.damage;
...
static bool compare_targets(const scanned_target& first, const scanned_target& second)
{
	...
	auto i = 0; i < 8; i++;
	switch (g_cfg.ragebot.weapon[i].selection_type) {
	case 0:
...
{
	auto i = 0; i < 8; i++;
	if (g_cfg.ragebot.weapon[i].selection_type)
...
bool hit_chance::can_hit(adjust_data* log, weapon_t* weapon, Vector angles, int hitbox) {
	...
	Vector start_scaled = { };
	const auto hitchance_cfg = g_cfg.ragebot.weapon[i].hitchance_amount;
